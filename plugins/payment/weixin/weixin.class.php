...
{    
    ...
    function getpay($order_id,$source){
        ...
        elseif($source=='box'){
        	...
        	$go_url = 'http://v.yykddn.com/kuaidi/send/check?order_id='.$order_id.'&source='.$source;
        	$back_url = 'http://v.yykddn.com/pay/payment/kuaidi?order_id='.$order_id.'&source='.$source;
        	$order = M('kd_order_sz')->where('order_id',$order_id)->find();
        	$body= "支付顺职(".$order['consignee'].")订单：".$order['order_sn'];
        	$Notify_url = SITE_URL.'/pay/payment/notifyUrl/source/'.$source.'/order_id/'.$order_id.'/pay_code/weixin';
        }
