...
function update_pay_status_diy($order_sn,$source,$ext=array())
{
    ...
    if($source=='kuaidi'){
        ...
        M('kd_order')->where("order_sn", $order_sn)->save($updata);
    }
    
    if($source=='box'){
        // 找出对应的订单
        $order = M('kd_order_box')->where("order_sn",$order_sn)->find();
        // 修改支付状态  已支付
        $updata = array('pay_status'=>1,'pay_time'=>time());
        if(isset($ext['transaction_id'])) $updata['transaction_id'] = $ext['transaction_id'];
        if(isset($ext['out_trade_no'])) $updata['out_trade_no'] = $ext['out_trade_no'];
        M('kd_order_box')->where("order_sn", $order_sn)->save($updata);
    }
    
    
    if($source=='ji'){
    	// 找出对应的订单
    	$order = M('kd_order_ji')->where("order_sn",$order_sn)->find();
    	// 修改支付状态  已支付
    	$updata = array('pay_status'=>1,'pay_time'=>time());
    	if(isset($ext['transaction_id'])) $updata['transaction_id'] = $ext['transaction_id'];
    	if(isset($ext['out_trade_no'])) $updata['out_trade_no'] = $ext['out_trade_no'];
...
function access_token(){
    $token = M('wx_user')->where(array('id'=>25))->find();
    $appid = $token['appid'];
