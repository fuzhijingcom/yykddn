<?php
namespace app\shuadan\controller;
...
class Xiadan extends MobileBase {
    ...
    public function xiadan_add(){
      ...
      //  $address = M('UserAddress')->where("address_id", $address_id)->find();
       ...
       // if($address['sushe']==null){
        ...
        header("Location: ".U('kuaidi/send/chenggong',array('order_id'=>$order_id,'source'=>'ss' )));
    ...
    public function caonima(){
        ...
        $user = M('users_qiang')->where(array('is_validated'=>1,'tuisong'=>1,'qun'=>1 ))->field('openid')->select();
        $c = count($user);
        
        $user1 = M('users_qiang')->where(array('is_validated'=>1))->field('openid')->select();
        $c1 = count($user1);
        
        $condition['type'] = array('eq','ss');
        $condition['order_id'] = array('gt',13904);
        
        $shuadan = M('kd_order')->where($condition)->order('order_id desc')->field('order_id,consignee,mobile,address,user_note')->select();
        dump($shuadan);
