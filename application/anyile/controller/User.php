...
//        $this->success($data['msg']);
    ...
    public function add_address()
    {
        if (IS_POST) {
            ...
            if ($data['status'] != 1){
                $this->error($data['msg']);
            }elseif(I('post.source') == 'an') {
                header('Location:' . U('anyile/xiadan/xiadan', array('address_id' => $data['result'],'type'=>I('type'))));
                exit;
            }

            $this->success($data['msg'], U('/anyile/User/address_list'));
            exit();
        }
    ...
    public function edit_address()
    {
        ...
        if (IS_POST) {
            ...
            if ($_POST['source'] == 'an') {
                header('Location:' . U('anyile/xiadan/xiadan', array('address_id' => $id,'type'=>I('type'))));
                exit;
            }else
                $this->success($data['msg'], U('/anyile/User/address_list'));
            exit();
    ...
    public function set_default()
    {
        ...
        if ($source == 'an') {
            ...
            header("Location:" . U('anyile/User/address_list'));
        }
    }
